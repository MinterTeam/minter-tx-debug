    ...
    <div class="u-grid u-grid--small u-grid--vertical-margin">
        <div class="multisig-signature__cell u-cell" v-for="(v, index) in $v.list.$each.$iter" :key="index">
            <label class="form-field multisig-signature__field" :class="{'is-error': v.signature.$error}">
                ...
                ></textarea>
                <span class="form-field__label">Signature #{{ Number(index) + 1 }}</span>
            </label>
            ...
            <span class="form-field__error" v-if="v.signature.$dirty && !v.signature.required">Enter signature</span>
        </div>
        <div class="u-cell u-text-left">
            <button class="button button--ghost-black" type="button" @click="addParticipant">
                + Add signature
            </button>
        </div>
    </div>
